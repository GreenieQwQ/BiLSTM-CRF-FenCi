...
torch.manual_seed(1)
LSTM_PAD_METHOD = False  # f——使用方法1 t——使用方法2
...
class BiLSTM_CRF(nn.Module):
    ...
    def _get_lstm_features(self, sentences, masks, pad=LSTM_PAD_METHOD):
        # 使用词向量输入
...
def train(HIDDEN_DIM=200, dir_path="../data/model/batch-blstm-crf/", mod=BiLSTM_CRF, b_size=500, use_cuda=False):
    ...
    fenci_result_train_path = batchDir + "/train_result.utf8"
    ...
    # 加载最优模型
    model.load_state_dict(torch.load("../data/model/batch-blstm-crf/bestModel.pt"))
    ...
    writeFenCiResult(trSet.x_data, pre_tags, fenci_result_train_path)


def main(args):
    train(b_size=args.batch_size, use_cuda=args.usecuda)


if __name__ == "__main__":
    # 命令行启动的方法
    parser = argparse.ArgumentParser()
    parser.add_argument('batch_size', type=int, help="The size of the batch.")
    parser.add_argument('--usecuda', action="store_true", help="Whether use cuda to train.")
    parser.add_argument('--snd', action="store_true", default=False, help="Whether use Method 2 to deal "
                                                                          "with the padding in lstm layer.")
    args = parser.parse_args()
    print(args)
    LSTM_PAD_METHOD = args.snd
    main(args)
